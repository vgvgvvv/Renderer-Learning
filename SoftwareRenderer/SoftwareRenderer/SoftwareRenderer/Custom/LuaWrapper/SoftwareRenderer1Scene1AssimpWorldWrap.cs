...
namespace UniToLua
{
    ...
    public class SoftwareRenderer1Scene1AssimpWorldWrap
    {
        
        public static void Register(UniLua.ILuaState L)
        {
			L.BeginClass(typeof(SoftwareRenderer.Scene.AssimpWorld), typeof(SoftwareRenderer.Core.World));
			L.RegFunction("New", _CreateSoftwareRenderer1Scene1AssimpWorld);
        ...
        private static int _CreateSoftwareRenderer1Scene1AssimpWorld(UniLua.ILuaState L)
        {
			if(L.CheckNum(0))
			{
				L.PushAny<SoftwareRenderer.Scene.AssimpWorld>(new SoftwareRenderer.Scene.AssimpWorld());
				return 1;
			}
			L.L_Error("call AssimpWorld constructor args is error");
			return 1;
        }
        
        private static int get_Objects(UniLua.ILuaState L)
        {
			var obj = (SoftwareRenderer.Scene.AssimpWorld) L.ToUserData(1);
        ...
        {
			if(L.CheckNum(2) && L.CheckType<SoftwareRenderer.Scene.AssimpWorld, object>(1))
			{
				bool result;
				var obj = (SoftwareRenderer.Scene.AssimpWorld) L.ToUserData(1);
